...
#root_directory_id = 'c2c0f38f0eb14650b12c35580fb681d1' #S2P L4s
...
def add_directory_content(directory_id, level_id, hierarchy_id):
    ...
    try:
            ...
            if(entry['rel'] == 'dir') or (entry['rel'] == 'mod'):
                ...
                if entry['rel'] == 'dir':
                    ...
                    if directoryName == 'Master Data Management' or directoryName == 'Plan to Perform (P2P)' or directoryName == 'L4 Process Flows':
                        ...
                        continue
                    if parentDirName == 'Hydrocarbon':
                        directoryName = parentDirName
                    report_data.append({'ETE Signavio Process Area': directoryName,
                                        ...
                                        'L3 Swimlane Role': '',
                                        'L4 Swimlane Role':'',
                                        'L3 Additional Participants':'',
                                        'L4 Additional Participants':'',
                                        'Security Composite Roles':'',
                                        'L3 Signavio ID':'' ,
                                        'L4 Signavio ID': entry['href'],
                                        'L3 UID':'',
                                        'Signavio Link':'',
                                        'ETE Description':''})
                ...
                if entry['rel'] == 'mod':
                    ...
                    if model_json.get('properties',{}).get('documentation', {}):
                    ...
                    for lane in lanedata:
                        ...
                        for index, task in enumerate(tasks):
                            ...
                            l3AddtnlParticipants = (','.join(elementName))
                            l3swimlaneRole = lane.get('properties',{}).get('name','')
                            ...
                            print(f"L3: {task.get('properties',{}).get('name','')},Lane:{l3swimlaneRole},addtnl:{elementName}")
                            ...
                            l3UID = task.get('properties',{}).get('meta-l3uid','')
                            ...
                            if task.get('stencil',{}).get('id','') == 'CollapsedSubprocess':
                                ...
                                if model_json2:
                                    ...
                                    element = 'Addntl'
                                    elementJSONName = 'processparticipant'
                                    pullConnectorData(model_json2)
                                    elementDict2 = createTargetElementDict(model_json2,element, elementJSONName)
                                    
                                    lanedata2 = filter_childshapes(model_json2, filter_list=['Lane'])
                                    for lane2 in lanedata2:
                                        
                                        addtnlParticipantsAvailable = False #Check whether addtnl role is available (need to get it through the resource ID)
                                        connectorDict = ''
                                        l4Tasks = filter_childshapes(lane2, filter_list =['Task'])
                                        if l4Tasks:
                                            for index, l4Task in enumerate(l4Tasks):
                                                ...
                                                if elementDict2 is None:
                                                    continue
                                                    
                                                
                                                for elementDetail in elementDict2:
                                                    
                                                    
                                                    outgoingIDDict = elementDict2[elementDetail]['Outgoing ID']
                                                    ...
                                                    for oidEntry in outgoingIDDict:
                                                        if oidEntry:
                                                            ...
                                                            #check whether that connector oid matches with the L3 rid
                                                            if (oidEntry['OID-Task'] == task.get('resourceId','')):
                                                                elementName.append(elementDict2[elementDetail]['Element Name'])
                                                                print(f"L3: {task.get('properties',{}).get('name','')},{elementName}")
                                                                
                                                l4AddtnlParticipants = (','.join(elementName))
                                                l4swimlaneRole = lane2.get('properties',{}).get('name','')
                                                
                                                print(f"L3: {task.get('properties',{}).get('name','')},Lane:{l4swimlaneRole},addtnl:{elementName}")
                                                
                                                #--Needs to be converted to a function accordingly----
                                                
                                                l4resource_id = l4Task.get('resourceId','')
                                                task_id = str(tmp_id) + str(index+1) + '.' 
                                                ...
                                                if (len(workdayL0)) > 0:
                                                    report_data.append({'ETE Group (Signavio Process Flow)': e2eName,
                                                                        ...
                                                                        'L3 Swimlane Role': l3swimlaneRole,
                                                                        'L4 Swimlane Role':l4swimlaneRole,
                                                                        'L3 Additional Participants':l3AddtnlParticipants,
                                                                        'L4 Additional Participants':l4AddtnlParticipants,
                                                                        'Composite Role':securitycompositeroles,
                                                                        'L3 Signavio ID': resource_id,
                                                                        'L4 Signavio ID': l4resource_id,
                                                                        'L3 UID':l3UID,
        ...
        if parentDirName != 'Hydrocarbon':
            create_excel_report('L4 Report backup', report_data, save_directory='output', timezone='local')
                ...
                if entry['rel'] == 'mod':
                    ...
                    for lane in lanedata:
                        ...
                        for index, task in enumerate(tasks):
                            ...
                            if task.get('stencil',{}).get('id','') == 'CollapsedSubprocess':
                                tasktype = task.get('properties',{}).get('meta-tasktype','')
                            #logger.debug(tasktype) #activelogger
                            ...
                            glossary_link=task.get('glossaryLinks',{}).get('name','')
                            if(glossary_link):
                                logger.debug('Glossary Linkss: ' + glossary_link[0]) #activelogger
                            else:
                                logger.debug('No dictionary item') #activelogger
                            glossary_results=''
                            if(glossary_link):
                                glossary_link=glossary_link[0][1:]
                                glossary_info=glossary_link+'/info'
                                logger.debug("Getting glos data")
                                
                                glossary_hireachy=get_all_sub_directories(spm,glossary_info)
                                logger.debug("glos data extraction completed")
                                #logger.debug(glossary_hireachy) #activelogger
                                if(glossary_hireachy):
                            ...
                            if (len(workdayL0)) > 0:
                                ...
                                report_data.append({'Process Area': directoryName,
                                                    ...
                                                    'Signavio Link': 'https://app-us.signavio.com/p/hub' + e2eLink,
                                                    'Model Description':modelDescription})
                                #logger.debug(report_data, type(report_data))
        
        #if parentDirName != 'Hydrocarbon':
            #create_excel_report('L3 Report backup', report_data, save_directory='output', timezone='local')
...
def get_all_sub_directories(spm, directoryID):
    try:
        ...
        logger.debug("SPM Call executed")
